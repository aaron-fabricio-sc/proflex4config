 ...
 * @requires jquery
 * @requires knockout
 * @REMARK: As of ProFlex4/UI 2.0/00 we updated shouldTransition() due to a transition bug when same view AND 
            the bindAndShow() function to use a promise to make the composition.finalize in line 533
 */
define(['durandal/system', 'durandal/viewLocator', 'durandal/binder', 'durandal/viewEngine', 'durandal/activator', 'jquery', 'knockout', 'extensions'], function (system, viewLocator, binder, viewEngine, activator, $, ko, ext) {
    var dummyModel = {},
    ...
    function shouldTransition(context) {
        if (system.isString(context.transition)) {
            if (context.activeView || context.skipTransitionOnSameViewId) {
    ...
    composition = {
        ...
        bindAndShow: function (child, element, context, skipActivation) {
            ...
            tryActivate(context, function () {
                var bindingPromise = ext.Promises.Promise.resolve();
                if (context.parent.__composition_context == context) {
                    ...
                    if (context.preserveContext && context.bindingContext) {
                        if (context.composingNewView) {
                            ...
                            bindingPromise = binder.bindContext(context.bindingContext, child, context.model, context.as);
                        }
                    } else if (child) {
                        ...
                        if (currentModel != modelToBind) {
                            ...
                            bindingPromise = binder.bind(modelToBind, child);
                        }
                    }
                    bindingPromise.then(() => composition.finalize(context, element));
                } else {
                    endComposition(context, element);
                }
