 ...
 * @requires system
 * @requires knockout
 * @REMARK: As of ProFlex4/UI 2.0/00 we updated the doBind() function to deliver a promise instead of a plain boolean
 */
define(['durandal/system', 'knockout', 'extensions'], function (system, ko, ext) {
    var binder,
    ...
    function doBind(obj, view, bindingTarget, data){
        return ext.Promises.promise(function(resolve, reject) {
            ...
            try {
                ...
                var bindingHookPromise = (instruction && instruction.then) ? instruction : ext.Promises.Promise.resolve(instruction);
                bindingHookPromise.then(function(instruction) {
