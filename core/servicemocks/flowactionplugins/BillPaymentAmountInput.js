...
$MOD$ BillPaymentAmountInput.js 4.3.1-200512-21-e0b258da-1a04bc7d
 */

/**
 * @module flowactionplugins/BillPaymentAmountInput
 */
define(["extensions"], function(ext) {
    "use strict";
    console.log("AMD:BillPaymentAmountInput");
        ...
        onGuiResult: async function(context) {
            try {
                if(context.action === "CONFIRM") {
                    ...
                    await _dataService.setValues("CCTAFW_PROP_TRANSACTION_AMOUNT", result["CCTAFW_PROP_UI_INPUT_FIELD_CONTENT_UNFORMATTED[0]"], null);
                }
            } catch(e) {
                throw `FlowAction plug-in BillPaymentAmountInput::onGuiResult has been failed ${e}`;
            }
        }
    };
});
