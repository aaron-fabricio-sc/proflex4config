...
$MOD$ OrderAmountInput.js 4.3.1-200512-21-e0b258da-1a04bc7d
 */

/**
 * @module flowactionplugins/OrderAmountInput
 */
define(["extensions"], function(ext) {
    "use strict";
    console.log("AMD:OrderAmountInput");
        ...
        onGuiResult: async function(context) {
            try {
                if(context.action === "CONFIRM") {
                    ...
                    await _dataService.setValues("CCTAFW_PROP_CUSTOMER_INPUT_VARIABLE[47]", result["CCTAFW_PROP_UI_INPUT_FIELD_CONTENT_UNFORMATTED[0]"], null);
                }
            } catch(e) {
                throw `FlowAction plug-in OrderAmountInput::onGuiResult has been failed ${e}`;
            }
        }
    };
});
